/**
 * @file page.tsx
 * @description Candidate registration page (`/signup`).
 */
...
export default function SignupPage() {
  ...
  const [name, setName] = useState("Staff Engineer");
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col justify-between items-center p-6 text-neutral-200 font-sans">
      ...
      <header className="w-full max-w-5xl flex items-center justify-between py-4 border-b border-white/10">
        ...
        <span className="font-mono text-[10px] text-neutral-500 uppercase">CANDIDATE INITIALIZATION</span>
      </header>

      {/* Signup Card */}
      <main className="w-full max-w-md border border-white/10 bg-[#0d0d0d] p-8 space-y-6 animate-fade-in-up">
        <div className="space-y-1 text-center">
          <div className="inline-flex items-center justify-center p-2.5 border border-white/10 bg-white/[0.02] text-emerald-400 mb-2">
            <UserPlus className="w-5 h-5" />
          </div>
          <h1 className="text-xl font-black uppercase tracking-tight text-white font-sans">
            INITIALIZE CANDIDATE SPEC
          </h1>
          <p className="text-xs font-mono text-neutral-400">
            MAP YOUR PROFILE AGAINST COMPUTABLE DAG ARCHITECTURES
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4 font-mono text-xs">
          <div className="space-y-1">
            <label className="text-[10px] uppercase text-neutral-400 tracking-wider">FULL NAME</label>
            <input
              type="text"
              value={name}
          ...
          <div className="space-y-1">
            <label className="text-[10px] uppercase text-neutral-400 tracking-wider">TARGET ARCHITECTURE TITLE</label>
            <input
              type="text"
              value={targetRole}
          ...
          <div className="space-y-1">
            <label className="text-[10px] uppercase text-neutral-400 tracking-wider">SECURE PASSWORD</label>
            <input
              type="password"
              value={password}
          ...
          >
            {isLoading ? "CREATING PROFILE..." : "LAUNCH ARCHITECTURE"} <ArrowRight className="w-4 h-4" />
          </button>
        </form>
      ...
      <footer className="font-mono text-[10px] text-neutral-600">
        DIRECTED ACYCLIC GRAPH RECOMMENDATIONS READY
      </footer>
    </div>
  );
}
